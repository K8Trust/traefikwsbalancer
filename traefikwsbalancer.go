...
func (b *Balancer) handleWebSocket(rw http.ResponseWriter, req *http.Request, targetService string) {
	...
	// Clone and preserve all headers
	cleanHeaders := make(http.Header)
	reqHeaders := req.Header.Clone()

	// First, copy all auth headers exactly as-is
	...
	// Then handle WebSocket headers
	wsKey := reqHeaders.Get("Sec-WebSocket-Key")
	if wsKey != "" {
		cleanHeaders.Set("Sec-WebSocket-Key", wsKey)
	}

	// Copy remaining headers
	for k, v := range reqHeaders {
		k = strings.TrimSpace(k)
		if _, exists := cleanHeaders[k]; !exists {
			switch strings.ToLower(k) {
			case "upgrade", "connection", "sec-websocket-version", "sec-websocket-protocol":
				continue // Skip WebSocket headers
			default:
				cleanHeaders[k] = v
			}
		}
	...
	for retries := 0; retries < 3; retries++ {
		log.Printf("[DEBUG] Connection attempt %d to %s with headers: %+v", retries+1, targetURL, cleanHeaders)
		...
		log.Printf("[WARN] Retry %d failed: %v", retries+1, err)
		if resp != nil {
			body, _ := io.ReadAll(resp.Body)
			log.Printf("[WARN] Response: status=%d, body=%s, headers=%v", resp.StatusCode, string(body), resp.Header)
			resp.Body.Close()

			// Check for 403 status code.
			if resp.StatusCode == http.StatusForbidden {
				if retries < 2 {
					log.Printf("[WARN] Received 403 response; retrying...")
				} else {
					// Last retry; forward 403 response.
				...
				// For other errors, forward response on final attempt.
				for k, v := range resp.Header {
					rw.Header()[k] = v
				}
				rw.WriteHeader(resp.StatusCode)
	...
	go func() {
		...
		for {
			...
			default:
				...
				log.Printf("[TRACE] Client -> Backend: type=%d, size=%d bytes", messageType, len(message))
	...
	go func() {
		...
		for {
			...
			default:
				...
				log.Printf("[TRACE] Backend -> Client: type=%d, size=%d bytes", messageType, len(message))
	...
	log.Printf("[DEBUG] Final connection count for service %s: %d", targetService, b.getServiceConnections(targetService))
	
	// Add delay before potential reconnect
